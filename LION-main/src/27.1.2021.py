...
functions = ['write','if','var','roar',"input","int","string", "savedVar", "while","random","function", "savedVarDelete", ]
keywords = ["//"]
variables = {}
saved_variables = {}
...
class JSON_saver:
  def __init__(self,fileInputClass):
    self.file_ = fileInputClass.fileName
  
  def read(file_):
    
    f = open("data.LIONDATA","rb")
    return pickle.load(f) 
  
  def writeDict(dictionary):

    saved_json = pickle.dumps(dictionary)
    f = open("data.LIONDATA", "wb")
    f.write(saved_json)
    f.close()
  ...
  def run(self,command_list,mode):
    ...
    for i in range(len(command_list)):
      ...
      if (current_command.command == "savedVar"):
        ...
        saved_variables[var_name] = var_val
...
saved_variables = JSON_saver.read(settings["DATAFILE"])
...
JSON_saver.writeDict(saved_variables)
