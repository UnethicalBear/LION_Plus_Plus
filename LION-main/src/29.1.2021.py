...
global variables,toknum,functions
declaredfunctions = {}
...
functions = ['write','if','var','roar',"input","int","string", "savedVar", "while","random","function", "savedVarDelete", "for", "system"]
keywords = ["//"]
variables = {}
saved_variables = {}
...
class Token:
  def __init__(self, type, value=None,level=None):
    self.type = type
    self.value = value
    self.level = level
  def __repr__(self):
  ...
  def make_command(self, text):
    ...
    for i in range(len(text)):
      ...
      if text[i] == '"' and isString == True:
        isString = False
        command_parts.append(Token(TO_STRING, current_text_list))
        current_text_list = ""
        continue
  ...
  def make_tokens(self,tok_list):
    ...
    for i in range(len(tok_list)):
      ...
      if self.current_tok.type != TO_STRING:
        ...
        if gettype == True:
          self.token_return_list[i] = Token(TO_INT, newvalue)
        if self.current_tok.value in functions:
          self.token_return_list[i] = Token(TO_FUNCTION, self.current_tok.value)
          continue
      
        if self.current_tok.value in comparators:
           self.token_return_list[i] = Token(TO_COMPARATOR, self.current_tok.value)
           continue
        if self.current_tok.value in keywords:
          self.token_return_list[i] = Token(TO_KEYWORD, self.current_tok.value)
          continue
        if self.current_tok.value in characters:
          if self.current_tok.value != "":
            self.token_return_list[i] = Token(TO_SPECALCHARACTERS, self.current_tok.value)
        if self.current_tok.type == TO_EOL:
          self.line += 1
  ...
  def parse(self,tok):
    ...
    for i in range(len(toks)):
      ...
      if toks[i].type == "BUILTIN_FUNCTION":
        if toks[i].value == "input":
          ...
          while linetrue == False:
            ...
            if toks[toknum].type == TO_RT and toks[toknum].value != "" and " " not in toks[toknum].type:
              
              commandcontents += "{}"
              variablenames.append(toks[toknum].value)
            if toks[toknum].type == TO_STRING:
              commandcontents += (toks[toknum].value)
            if toks[toknum].type == TO_EOL:
              linetrue = True
              self.pos.newline()
              if toks[i].value == "input":
                commandlist.append(BUILTIN_FUNCTION("input",[commandcontents,variablenames]))
                continue
            toknum += 1
        ...
        if toks[i].value == "for":
          ...
          commandlist.append(BUILTIN_FUNCTION("for",[
            [l,c,r],
            [executelist]
            ]))
        ...
        if toks[i].value == "var":
          ...
          while linetrue == False:
            ...
            if toks[toknum].type == TO_EOL:
              ...
              commandlist.append(BUILTIN_FUNCTION("var",[varname,commandcontents]))
      ...
      if toks[i].value == "string":
        commandlist.append(BUILTIN_FUNCTION("string",[toks[i+1].value]))
      if toks[i].value == "int":
        commandlist.append(BUILTIN_FUNCTION("int",[toks[i+1].value]))
...
class Interpreter:
  ...
  def parsevalues(self,values):
        ...
        if isinstance(value,list):
          ...
          if value.type == TO_RT:
            if value.value in variables:
              assignvalue.append(variables[value.value])
    ...
    return output
  def run(self,command_list,mode):
    md = mode
    ...
    for i in range(len(command_list)):
      ...
      if current_command.command == 'input':
        ...
        if "{}" in temp[0]:
          ...
          for i in range(len(temp[1])):
            ...
            output = output.replace("{}",str(variables.get(varnames[i]).value),1)
            
        returner = Token(TO_STRING,input(output))
      ...
      if current_command.command == 'int':
        args = current_command.args[0]
        variables[args] = Token(TO_INT,int(variables[args].value))
      if current_command.command == 'string':
        args = current_command.args[0]
        variables[args] = Token(TO_INT,str(variables[args].value))
      ...
      if current_command.command == "for":
        ...
        if md == "auto":
          ...
          if stop.type == TO_RT and increment.type == TO_RT:
            if stop.value == increment.value:
              Error("Syntax", "Unknown", fileControl.fileName, "both values of an auto loop may not be the same.")

          if stop.type != TO_RT:
            Error("Syntax","Unknown", fileControl.fileName, "assigner cannot be a string")
          else:
            assigner = stop.value
            # print(assigner)
        ...
        if md == "std":
          ...
          if start.type == TO_INT:
            ...
            try:
              c1 = variables[start.value.strip()].value
              variable = start.value.strip()
            except: 
              c1 = None
              print("ERROR")
          ...
          for i in range(c1, c2, c3):
            self.selfrun(code)
            variables[variable].value = variables[variable].value +c3
      ...
      if current_command.command == 'var':# interprit the raw token as variables and get the values. something to do tomoz. :) i want to die btw
        ...
        variables[current_command.args[0]] = assignvalue[0]        
      ...
      if current_command.command == "if":
        ...
        for i in range(len(conditions)):
          if conditions[i].type == TO_RT:
              ...
              conditions[i] = variables.get(conditions[i].value)
      ...
      if current_command.command == "while":
        ...
        def makeConditionResult(l,c,r):
          ...
          if l.type != TO_STRING or TO_INT or TO_FLOAT:
            
            l = variables[l.value.strip()]
            l = l.value
...
fileControl.getFile()
postition = Postition(fileControl.fileName)
